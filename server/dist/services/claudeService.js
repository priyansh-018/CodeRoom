 ...
 */
async function fetchLiveLeetCodeQuestion(topic = 'Arrays & Hashing', difficulty = 'Medium', language = 'javascript', excludeTitles = []) {
    try {
        ...
        const rawQuestions = listData?.data?.problemsetQuestionList?.questions || [];
        const qList = rawQuestions.filter((q) => !q.isPaidOnly);
        const total = listData?.data?.problemsetQuestionList?.total || qList.length;
        if (qList.length === 0)
            return null;
        const isExcluded = (title, slug) => {
            if (!title)
                return false;
        ...
        const uncompletedQList = qList.filter((q) => !isExcluded(q.title, q.titleSlug));
            ...
            try {
              ...
              problemsetQuestionList: questionList(categorySlug: $categorySlug, limit: $limit, skip: $skip, filters: $filters) {
                ...
                if (pageRes.ok) {
                    ...
                    const pQuestions = (pageData?.data?.problemsetQuestionList?.questions || []).filter((q) => !q.isPaidOnly);
                    const pUncompleted = pQuestions.filter((q) => !isExcluded(q.title, q.titleSlug));
        ...
        if (!selectedItem || !selectedItem.titleSlug)
            return null;
        ...
        if (q.codeSnippets && Array.isArray(q.codeSnippets)) {
            for (const snip of q.codeSnippets) {
                if (snip.langSlug === 'javascript')
                    starterCode.javascript = snip.code;
                else if (snip.langSlug === 'typescript')
                    starterCode.typescript = snip.code;
                else if (snip.langSlug === 'python3' || snip.langSlug === 'python')
                    starterCode.python = snip.code;
                else if (snip.langSlug === 'cpp')
                    starterCode.cpp = snip.code;
                else if (snip.langSlug === 'java')
                    starterCode.java = snip.code;
                else if (snip.langSlug === 'csharp')
                    starterCode.csharp = snip.code;
                else if (snip.langSlug === 'golang')
                    starterCode.golang = snip.code;
                else if (snip.langSlug === 'rust')
                    starterCode.rust = snip.code;
                else if (snip.langSlug === 'mysql' || snip.langSlug === 'postgresql')
                    starterCode.sql = snip.code;
            }
        }
        return {
...
async function generateQuestion(topic = 'Arrays & Hashing', difficulty = 'Medium', language = 'javascript', excludeTitles = []) {
    ...
    const isDevOps = langLower === 'bash' ||
        ...
        lower.includes('bash') ||
        lower.includes('pod');
    const isSysDesign = lower.includes('system design') ||
...
Return ONLY a valid, raw JSON object without markdown fences, with exactly this schema:
{
  ...
  "starterCode": {
    "${language}": "// Starter code implementation template"
        ...
        const topicFiltered = candidateQuestions.filter(q => q.topic?.toLowerCase().includes(lower) ||
...
function evaluateCodeStrictly(question, code, language, isDisqualified = false) {
...
async function analyzeCode(question, code, language, isDisqualified = false) {
    if (anthropic && anthropicApiKey) {
        try {
