...
export const sendSignupOtp = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existingInterviewer = await prisma.interviewer.findUnique({ where: { email: cleanEmail } });
    const existingCandidate = await prisma.candidate.findUnique({ where: { email: cleanEmail } });
...
export const verifyOtpAndRegister = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (new Date() > otpRecord.expiresAt) {
      await prisma.emailOtp.delete({ where: { email: cleanEmail } }).catch(() => {});
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let user: any = await prisma.interviewer.findUnique({ where: { email: cleanEmail } });
    let role = 'HOST';

    if (!user) {
      user = await prisma.candidate.findUnique({ where: { email: cleanEmail } });
      role = 'CANDIDATE';
    }
...
export const updateProfile = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const skillsString: string | null | undefined = skills !== undefined
    ...
    if (isInterviewer) {
      ...
      updated = await prisma.candidate.update({
        ...
        data: {
          ...
          skills: (skillsString !== undefined ? (typeof skillsString === 'string' ? skillsString : JSON.stringify(skillsString)) : existing.skills) as any,
